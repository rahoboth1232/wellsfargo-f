    ...
    <header className="flex flex-col gap-3 px-16 py-4 border-b border-gray-200">
      <div className="flex justify-between items-center">
        <nav className="flex gap-6">
          {NAV_LINKS.map((l) => (
            <a key={l} href="#" onClick={handleSubmit} className="text-sm text-gray-800 hover:text-[#005eb8] font-medium transition-colors">
              {l}
            </a>
          ))}
        </nav>
      ...
      <nav className="flex gap-6 overflow-x-auto pb-1">
        {SUB_NAV.map((l) => (
          <a key={l} href="#" onClick={handleSubmit} className="text-sm text-gray-700 whitespace-nowrap hover:text-[#005eb8] transition-colors">
            {l}
          </a>
        ))}
      </nav>
